...
function getCityAndState(person) {
  let curCity = person.address.city;
  let curState = person.address.state;
...
function getFullAddress(person) {
  let curStreet = person.address.street;
  let curUnit = person.address.unit;
  let curCity = person.address.city;
  let curState = person.address.state;
  let curZip = person.address.zip;
  return `${curStreet} ${curUnit}, ${curCity}, ${curState}, ${curZip}`;
}
/**
 * getFlatObject()
 * ---------------------
