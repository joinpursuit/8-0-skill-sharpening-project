...
function getFullName(person) {
  let full = `${person.names.first} ${person.names.middle} ${person.names.last}`;
  return full;
}
...
function getCityAndState(person) {
  let cs = `${person.address.city}, ${person.address.state}`;
  return cs;
}
...
function getFullAddress(person) {
  let full = `${person.address.street} ${person.address.unit}, ${person.address.city}, ${person.address.state}, ${person.address.zip}`;
  return full;
}
...
function getFlatObject(person) {
  person = { first: person.names.first,
