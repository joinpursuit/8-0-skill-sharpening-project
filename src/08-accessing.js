...
function getFullAddress(person) {
  const { street, city, state, unit, zip } = person.address;
...
function getFlatObject(person) {
  const result = {};
  for (const item in person.names) {
    result[item] = person.names[item];
  }

  for (const item in person.address) {
    result[item] = person.address[item];
  }
