...
function getFullAddress(person) {
  ...
  const pcity= person.address.city
  const state= person.address.state
  const zip= person.address.zip
  return `${street} ${unit}, ${pcity}, ${state}, ${zip}`
}
...
function getFlatObject(person) {
  const myObj= {}
   myObj["first"]= person.names.first
   myObj["middle"]= person.names.middle
   myObj["last"]= person.names.last
   myObj["street"]= person.address.street
   myObj["unit"]= person.address.unit
   myObj["city"]= person.address.city
   myObj["state"]= person.address.state
   myObj["zip"]= person.address.zip
   myObj["numbers"]= person.numbers
  return myObj
