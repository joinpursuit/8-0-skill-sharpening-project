...
function getLastPhoneNumber(person) {
  lastNumber = person.numbers[person.numbers.length - 1];
  return lastNumber;
}
...
function getFullAddress(person) {
  fullAddress = `${person.address.street} ${person.address.unit}, ${person.address.city}, ${person.address.state}, ${person.address.zip}`;
return fullAddress
}
