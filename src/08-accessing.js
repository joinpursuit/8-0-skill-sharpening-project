...
function getFullName(person) {
  x= person.names.first +" "+ person.names.middle + " "+ person.names.last
  return x
...
function getFullAddress(person) {
  ...
  // x= Object.values(person.address).join(` `)
  // return x

  x=person.address

                 
return x.street +" "+ x.unit +`,`+` `+ x.city+`,`+` `+x.state+`,`+` ` +x.zip
...
const x = person.names
const y = person.address
const z = person.numbers
