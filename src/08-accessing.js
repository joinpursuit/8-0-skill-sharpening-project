...
function getFullName(person) {
  const names = `${person.names.first} ${person.names.middle} ${person.names.last}`
  return names
}
...
function getCityAndState(person) {
  const cityAndState = person.address.city + ", " + person.address.state
  return cityAndState
}
...
function getFullAddress(person) {
  ...
  const fullAddress = `${person.address.street} ${person.address.unit}, ${person.address.city}, ${person.address.state}, ${person.address.zip}`;
...
function getFlatObject(person) {
  ...
  let newObj = {};
  // Define my loop
  for (const i in person) { // for in is for objects.
    // Acuumulating 
    // console.log(i) -> keys from the object 'person'
    if (person[i] !== person.numbers){
      for (const i2 in person[i]) {
        newObj[i2] = person[i][i2] 
     }
    } else {
      newObj[i] = person.numbers
    }
  }
