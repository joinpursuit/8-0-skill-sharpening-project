...
function getFlatObject(person) {
 ...
 newList.first = person.names.first
 newList.middle = person.names.middle
 newList.last = person.names.last
 
 
 //make copy of the objects key:value  
 newList.street = person.address.street
 newList.unit = person.address.unit
 newList.city = person.address.city
 newList.state = person.address.state
 newList.zip = person.address.zip
newList.numbers =[]
for(let i =0; i< person.numbers.length; i++){
 newList.numbers[i] = person.numbers[i]
}
  return newList
}
