...
function getFlatObject(person) {
  const flatPerson = {};
  // loop through names and add each key and value to flatPerson
  for (let key in person.names) {
    flatPerson[key] = person.names[key];
  }
  // loop through address and add each key and value to flatPerson
  for (let key in person.address) {
    flatPerson[key] = person.address[key];
  }
  flatPerson.numbers = person.numbers;
