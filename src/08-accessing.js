...
function getAddressStreet(person) {
  ...
  const forStreet = person.address.street
...
function getFirstPhoneNumber(person) {
  ...
  const phoneNumber = person.numbers[0]
...
function getLastPhoneNumber(person) {
  ...
  let person2 = person.numbers
  
  return person2[person2.length-1]
...
function getFullName(person) {
  ...
  const full_name = `${person.names.first} ${person.names.middle} ${person.names.last}`
...
function getFullAddress(person) {
  ...
  const  address = `${person.address.street} ${person.address.unit}, ${person.address.city}, ${person.address.state}, ${person.address.zip}`
