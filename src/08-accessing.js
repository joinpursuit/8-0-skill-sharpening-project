...
function getFullAddress(person) {
  return `${getAddressStreet(person)} ${person.address.unit}, ${getCityAndState(person)}, ${person.address.zip}`;
}
...
function getFlatObject(person) {
  let newObj = {};
  for(let o in person) {
    if(!Array.isArray(person[o])) {
      for(let io in person[o]) {
        newObj[io] = person[o][io];
      }
    } else {
        newObj[o] = person[o];
    }
