...
function createObjectWithValue(val) {
  const newObj = {};
  newObj["key"] = val; //key has to be in quotes//
  return newObj;
}
...
function createObjectWithKeyValuePair(key, val) {
  const object ={}
  object[key] = val //use bracket notation so it can be universal
  return object;
}
...
function addValueToObject(object, val) {
  object.name = val; //.name -->> key || val added value to key
  return object
...
function addKeyValuePairToObject(object, key, val) {
  object[key] = val; //bracket notation
  return object
}
...
function deleteKeyFromObject(object, key) {
  //use delete op
  delete object[key]; //bracket notation
  return object;
}
