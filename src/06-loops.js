...
function targetCount(array, target) {
  let result = 0// number of times target is found
  for(let i = 0; i < array.length; i++){
    if(array[i] === target){
      result += 1
    }
...
function cumulativeSum(array) {
  let newArray = []// need new array
  let num  = 0 // to ensure 1st integer is diplayed too

  for(i = 0; i< array.length; i++){
    num += array[i];// num = 5 in 1st iteration, then 5+ 2 in 2nd iteration, etc.
    newArray.push(num)
  }
  return newArray
}
