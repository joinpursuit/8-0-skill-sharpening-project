...
function rangeOdd(min, max) {
  ...
  //creating a empty array to put in the numbers
  const arr = []
  //creating a while loop to go through the numbers
  while (max >= min) {
  //checkinh if the number is even or not and if it is, its goin
  //to push it in to the array
    if (max % 2 === 1) { 
      arr.push(max)}
  //deletes 1 from the max
      max--
}
return arr
}
