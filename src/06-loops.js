const { evenOrOdd } = require("./03-control-flow");
...
function rangeSum(min, max) {
  ...
  for (let i= min; i <= max; ++i){
    // increase number within the loop each time it is put in the loop
    sum+=i;
    // sum of all numbers from min to max that is created through the loop
  }
  return sum
}
