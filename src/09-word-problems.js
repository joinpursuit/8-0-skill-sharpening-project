 ...
 * they receive a 10% discount. If the attendee is viewing the movie with a member, they receive a 20% discount. If both situations apply, 
 * they receive a 30% discount.
 *
 * Write an algorithm that will determine the price of a ticket based on the `priceInCents` of the ticket, the `age` of the attendee, and 
 * he membership status (i.e. `hasMembership`).
...
function applyDiscount(priceInCents, age, hasMembership) {
  // 10 or younger = 10% off
  // 65 or older = 10% off
  // if a member = 20 % off,
  // if both young & old + member = 30% off

  let thirtyPercentOff = priceInCents - (.30 * priceInCents);
  let twentyOff = priceInCents - (.20 * priceInCents);
  let tenPercentOff = priceInCents - (.10 * priceInCents);

  if (hasMembership){
    if (age <= 10){
      return thirtyPercentOff
    } else if (age >= 65) {
      return thirtyPercentOff
    } else {
      return twentyOff
 ...
 * compareLocations()
 * ---------------------
 * A shipping company is looking to make its deliveries more efficient by comparing the 
 * destinations of multiple deliveries. If the locations are similar, the packages may be 
 * able to be bundled together.
 * 
 ...
 * 
 * - If the street, city, state, and zip for both addresses are the same, return the string "Same 
 * building."
 ...
 * 
 * Keep in mind that an address could have the same street address or the same city but be in a 
 * different state. 
 * 
 ...
 *
 * Write an algorithm that adds a key of `status` to each object in an array of objects. Each object represents 
 * a single assignment submitted by a student.
 *
 * - If the assignment has a `kind` of `"PASS-FAIL"`, set the `status` value to `"PASSED"` if the `score.received` 
 * equals the `score.max`. Otherwise, set that `status` to be `"FAILED"`.
 * - If the assignment has a `kind` of `"PERCENTAGE"`, set the `status` value to be `"PASSED: <percentage>"` if the 
 * student scored at least 80.0%. The `<percentage>` should be set to one decimal place. If the student scored less 
 * than 80.0%, set the status to `"FAILED: <percentage>"`.
...
function gradeAssignments(assignments) {
  for (i = 0; i < assignments.length; i++){
    if (assignments[i].kind === 'PASS-FAIL'){
      ...
      let percentCalc = ((assignments[i].score.received / assignments[i].score.max) * 100).toFixed(1);

      if (percentCalc >= 80.0) {
        assignments[i].status = `PASSED: ${percentCalc}%`;
      } else { 
        assignments[i].status = `FAILED: ${percentCalc}%`;       
      }
    } else {
 ...
 * createLineOrder()
 * ---------------------
 * An airline wants to build an application that improves the boarding process for its customers. They want to have 
 * customers sign up in order of arrival, but prioritize those customers who have a membership.
 * 
 * Build an algorithm that takes in an array of objects, where each object represents a person. The order of the array 
 * is important; the person at index `0` arrived first while the person at index `1` arrived afterwards.
 * 
...
function createLineOrder(people) {
  ...
  for (i = 0; i < people.length; i++){
    if (people[i].hasMembership === true){
      priorityBoarding.push(people[i].name)
    } else {
      normalBoarding.push(people[i].name)
    }
  }
