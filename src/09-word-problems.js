 ...
 */
 function applyDiscount(priceInCents, age, hasMembership) {
  let discount= 0
  if (!hasMembership && (age<=10 || age>=64)){
    discount= priceInCents - (priceInCents * .1)
    return discount
  }
  else if (hasMembership && (age<=10 || age>=64)){
    discount= priceInCents - (priceInCents * .2)
    return discount
  }
  else if (age<=10 || age>=64)
  if (hasMembership){
    discount= priceInCents - (priceInCents * .3)
    return discount
  }
  return priceInCents
}
...
function getCartTotal(products) {
  let total = 0
  for (i=0; i< products.lenghth; i++){
    total = products.priceInCents[i]* products.quanity[i]
  }
