...
function applyDiscount(priceInCents, age, hasMembership) {
  if (hasMembership) {
    if (age <= 10 || age >= 65) {
      return priceInCents * 0.70;
    } else {
      return priceInCents * 0.80;
    }
  } else if (age <= 10 || age >= 65) {
    return priceInCents * 0.90;
  } else {
    return priceInCents;
  }
}
...
function getCartTotal(products) {
  ...
  for (let i = 0; i < products.length; i++) {
    price = products[i].priceInCents * 0.01 * products[i].quantity;
    total += price;
  };
...
function gradeAssignments(assignments) {
  for (i = 0; i < assignments.length; i++) {
    if (assignments[i].kind === "PERCENTAGE") {
      let score = assignments[i].score.received / assignments[i].score.max * 100;
      if (score >= 80) { 
