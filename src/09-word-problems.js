...
function applyDiscount(priceInCents, age, hasMembership) {
  if ((age >= 65) && (hasMembership)) {
    let discount = priceInCents - (priceInCents/100) * 30;
    return discount;
  }
    ...
    let discount = priceInCents - (priceInCents/100)  * 10;
    return discount;
  }
  else if (age <= 10) {
    let discount = priceInCents - (priceInCents/100)  * 10;
    ...
    let discount = priceInCents - (priceInCents/100) * 20;
    return discount;
  }
...
function getCartTotal(products) {
  ...
  for (let i = 0; i < products.length; i++) {
    final += products[i].priceInCents * products[i].quantity;
    }
 ...
 * TODO: ADDING A NEW KEY (status) to each object. What value are we giving it? 
 * if assignments[].kind = 'PASS-FAIL'
 * object.status = 'PASSED' if score.received === score.max
 * else object.status = 'FAILED'
 * 
 * if assignments[].kind = `"PERCENTAGE"`
 * object.status = `"PASSED: <percentage>"` if score.received >= 80.0%
 * 
 * if score.received < 80.0%
 * object.status= `"FAILED: <percentage>"`
 * decimal is on decimal place, so (/100).toFixed(1);
 * 
 * assignments[].kind is anything else
 * object.status = `"Score: score.recieved/score.max"
 * 
 ...
 * - If the assignment has a `kind` of `"PERCENTAGE"`, set the `status` value to be `"PASSED: <percentage>"` if the student scored at least
 * 80.0%. 
