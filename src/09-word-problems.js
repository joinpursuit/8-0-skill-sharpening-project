 ...
 * If the attendee is 10 years old or younger, 
 * or 65 years old or older, they receive a 10% discount. 
 * If the attendee is viewing the movie with a member, 
 * they receive a 20% discount. 
 * If both situations apply, 
 * they receive a 30% discount.
 *
 * Write an algorithm that will determine the price of a ticket 
 * based on the `priceInCents` of the ticket, the `age` of the attendee, 
...
function getCartTotal(products) {
 ...
 for (let i = 0; i < products.length; i++) {
   amountCents += products[i].priceInCents * products[i].quantity;
 }
 let amountDollars = amountCents / 100;
 return `$${amountDollars.toFixed(2)}`;
}
/**
 * compareLocations()
 * ---------------------
 * A shipping company is looking to make its deliveries more efficient 
 * by comparing the destinations of multiple deliveries. 
 * If the locations are similar, the packages may be able to be bundled together.
 * 
 * Write an algorithm that takes in two objects of similar shape, 
 * each object representing an address. Then, return a string that describes 
 * the relationship between those two addresses.
 * 
 ...
 * 
 * Keep in mind that an address could have the same street address or 
 * the same city but be in a different state. 
 * 
 ...
 * gradeAssignments()
 * ---------------------
 * An online learning management system needs a way to quickly add the current status 
 * to a list of assignments. Depending on the `kind` of assignment, 
 ...
 * set the `status` value to equal `"SCORE: <received>/<max>"`, where `<received>` 
...
function gradeAssignments(assignments) {
  ...
  for (let i = 0; i < assignments.length; i++) {
    ...
    if (assignment.kind === `PASS-FAIL`) {
      ...
      let scoredAmt = assignment.score.received / assignment.score.max * 100;
      if (scoredAmt >= 80) {
        assignment.status = `PASSED: ${scoredAmt.toFixed(1)}%`;
      } else {
        assignment.status = `FAILED: ${scoredAmt.toFixed(1)}%`;
      }
      } else {
 ...
 * createLineOrder()
 * ---------------------
 * An airline wants to build an application that improves 
 * the boarding process for its customers. 
 * They want to have customers sign up in order of arrival, 
 * but prioritize those customers who have a membership.
 * 
 * Build an algorithm that takes in an array of objects, 
 * where each object represents a person. 
 * The order of the array is important; the person at index `0` arrived first 
 * while the person at index `1` arrived afterwards.
 * 
 ...
 * @returns {string[]} An array of names, in order depending on the person's order 
 * in the original array and the person's priority.
 * 
 * EXAMPLE:
 *  const people = [
  ...
  for (let i = 0; i < people.length; i++) {
    person = people[i];
     if (person.hasMembership) {
       ...
       nonMem.push(person.name);
     }
  }
  let queueList = member.concat(nonMem);
  return queueList;
}
