...
function applyDiscount(priceInCents, age, hasMembership) {
  ...
  if(!hasMembership && (age <= 10 || age >= 65)){
    finalPrice = priceInCents - (priceInCents * .1)
     return finalPrice
  } else if (hasMembership === true && !(age <= 10 || age >= 65)){
    finalPrice = priceInCents - (priceInCents * .2)
   return finalPrice
  } else if(age <= 10 || age >= 65)
    if(hasMembership){
      finalPrice = priceInCents - (priceInCents * .3)
      return finalPrice
  } 
return priceInCents
...
function compareLocations(address1, address2) {
  if (address1.street === address2.street &&
    address1.city === address2.city && address1.state === address2.state &&
