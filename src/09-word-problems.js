...
function applyDiscount(priceInCents, age, hasMembership) {
 let totalPrice = 0;
  ...
  return totalPrice = priceInCents * 0.7
  }
  if(age <= 10 || age >= 65) {
    return totalPrice = priceInCents * 0.9
  }
  else if(hasMembership) {
    return totalPrice = priceInCents * 0.8
  }
  else {
    return priceInCents;
  }
 ...
 * Write an algorithm that will determine the total amount of all items in the cart. Make sure to multiply the `priceInCents` times the `quantity` 
 * to get the full cost of each product.
