...
function greet(name) {
  return `Hello, ${name}.`;
  //LITERAL TEMPLATE SAME AS: "Hello," + " " + name + "." CONCATANATION
};
...
function compareTypes(a, b) {
  if (typeof(a) === typeof(b)) {//treating typeof as a function and IT IS NOT... remove parenthesis around parameters a, b.
    return true;
  } else { //COULD BE WRITTEN W/OUT THE ELSE 
    return false;
  }
};//or return (typeof a === typeof b); typeof operand evaluates to True or False already
 ...
 *  //> -50
 */
function flipSign(a) {// return -(a) is also the correct code 
  if (!Number.length === 0) {//whatever number is (a)*-1
    return null 
  }
...
//flipSign(-25)//to debug always call function first
console.log(flipSign(-50))
// Will not debug if function not called and syntactically incorrect
//if (a * -1)// changes positive to negative
//return Math.abs(a) // changes negative to positive
...
function getFirstAndLastCharacter(a) {
  //if (a.length === 1) { // edgecase if only 1 string is inputted
    //return a[0]}
  
  return a[0] + a[a.length-1];
};//a at 0 index + a at [the last character] because (a) is a word. A word is just an array of characters

getFirstAndLastCharacter("k")
